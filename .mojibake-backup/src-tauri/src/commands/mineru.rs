...
fn file_name_from_path(path: &Path) -> Result<String, String> {
  path
    ...
    .ok_or_else(|| "无法读取 PDF 鏂囦欢鍚?.to_string())
}
...
fn update_selected_path(target: &mut Option<(i32, String)>, priority: i32, path: String) {
  ...
  if priority > target.as_ref().map(|(current_priority, _)| *current_priority).unwrap_or(0) {
...
fn api_error<T>(envelope: MineruApiEnvelope<T>, context: &str) -> String {
  ...
  format!("{}失败，code={}，message={}", context, envelope.code, message)
}
...
async fn parse_json_response<T: DeserializeOwned>(
  ...
  serde_json::from_str::<MineruApiEnvelope<T>>(&text)
    .map_err(|error| format!("解析{}响应 JSON 失败：{}，原始响应：{}", context, error, text))
}
...
fn read_zip_texts(zip_bytes: &[u8]) -> Result<ZipTexts, String> {
  ...
  let mut archive = ZipArchive::new(cursor).map_err(|error| format!("读取结果 zip 失败：{}", error))?;
  ...
  for index in 0..archive.len() {
    ...
    let middle_priority = if lower_name.ends_with("middle.json") || lower_name.contains("_middle") {
      3
    ...
    if content_priority > selected_content.as_ref().map(|(priority, _)| *priority).unwrap_or(0) {
      selected_content = Some((content_priority, text.clone()));
    }

    if middle_priority > selected_middle.as_ref().map(|(priority, _)| *priority).unwrap_or(0) {
      selected_middle = Some((middle_priority, text.clone()));
    }

    if markdown_priority > selected_markdown.as_ref().map(|(priority, _)| *priority).unwrap_or(0) {
...
fn extract_zip_files(zip_bytes: &[u8], extract_dir: &Path) -> Result<ExtractedZipPaths, String> {
  fs::create_dir_all(extract_dir)
    .map_err(|error| format!("创建 MinerU 解析目录失败：{}，{}", extract_dir.display(), error))?;

  let cursor = Cursor::new(zip_bytes);
  let mut archive = ZipArchive::new(cursor)
    .map_err(|error| format!("读取 MinerU zip 文件失败：{}", error))?;
  ...
  for index in 0..archive.len() {
    ...
    if file.is_dir() {
      fs::create_dir_all(&output_path)
        .map_err(|error| format!("创建 MinerU 目录失败：{}，{}", output_path.display(), error))?;
      continue;
    }

    if let Some(parent) = output_path.parent() {
      fs::create_dir_all(parent)
        .map_err(|error| format!("创建 MinerU 目录失败：{}，{}", parent.display(), error))?;
    }
    ...
    fs::write(&output_path, bytes)
      .map_err(|error| format!("写入 MinerU 条目失败：{}，{}", output_path.display(), error))?;
    ...
    update_selected_path(&mut content_json_path, content_priority, output_path_string.clone());
    update_selected_path(&mut middle_json_path, middle_priority, output_path_string.clone());
...
pub async fn run_mineru_cloud_parse(
  ...
  let upload_file_response = client
    ...
    .send()
    .await
    .map_err(|error| format!("上传 PDF 閸?MinerU 失败：{}", error))?;
  ...
  if !upload_file_response.status().is_success() {
    ...
    return Err(format!("上传 PDF 閸?MinerU 失败，HTTP 状态：{}，{}", status, text));
  }

  let start = Instant::now();
  let final_result = loop {
    if start.elapsed() > timeout {
      return Err(format!("等待 MinerU 云端解析超时，batch_id：{}", upload_data.batch_id));
    }

    let status_endpoint = format!("{}/extract-results/batch/{}", MINERU_API_BASE, upload_data.batch_id);
    ...
    let current_result = pick_extract_result(
      status_data.extract_result.into_vec(),
      &data_id,
      &file_name,
    )?;
    ...
    match current_result.state.as_str() {
      ...
      "failed" => {
        return Err(format!(
          "MinerU 云端解析失败：{}",
          current_result.err_msg.unwrap_or_else(|| "未知错误".to_string())
        ));
      }
  ...
  if !zip_status.is_success() {
    ...
    return Err(format!("下载 MinerU 结果 zip 失败，HTTP 状态：{}，{}", zip_status, text));
  }
...
pub fn run_mineru_placeholder(pdf_path: String) -> Result<String, String> {
  if pdf_path.trim().is_empty() {
    return Err("PDF 路径不能为空".to_string());
  }

  Ok(format!(
    "MinerU 扩展入口已预留，后续可在 Rust 鐏炲倹甯撮崗銉︽拱閸?Python、后端服务或远程 API銆傚綋鍓?PDF：{}",
    pdf_path
  ))
}
