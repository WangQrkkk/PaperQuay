...
fn extract_field_fragment(content: &str, key: &str) -> Option<String> {
  ...
  Some(content[value_start..value_end].trim().trim_end_matches(',').trim().to_string())
}
...
fn salvage_summary_content(content: &str, fallback_title: &str) -> Option<PaperSummary> {
  ...
  for key in SUMMARY_STRING_FIELDS {
    let value = extract_field_fragment(&cleaned, key).map(|fragment| decode_loose_string(&fragment));
...
fn normalize_summary(mut summary: PaperSummary) -> PaperSummary {
  if summary.title.trim().is_empty() {
    summary.title = "閺堫亜鎳￠崥宥堫啈閺?.to_string();
  }
  ...
  for field in [
    ...
    if field.trim().is_empty() {
      *field = "鏈湪褰撳墠鍐呭涓槑纭鏄庛€?.to_string();
    }
  }

  summary.key_findings =
    normalize_list(summary.key_findings, "閺堫亙绮犺ぐ鎾冲閸愬懎顔愭稉顓熷絹閻愮厧鍤弰搴ｂ€橀崣鎴犲箛閵?);
  summary.takeaways = normalize_list(summary.takeaways, "瀵ら缚顔呯紒鎾虫値閸樼喐鏋冩潻娑楃濮濄儳鈥樼拋銈堫啈閺傚洩纭€閻氼喓鈧?);
  summary.keywords = normalize_list(summary.keywords, "瀵板懓藟閸?);
...
fn parse_summary_content(content: &str, fallback_title: &str) -> Result<PaperSummary, String> {
  ...
  Ok(normalize_summary(materialize_summary(summary, fallback_title)))
}
...
fn parse_summary_content_resilient(
  ...
  for candidate in &candidates {
    match serde_json::from_str::<RawPaperSummary>(candidate) {
      Ok(summary) => {
        return Ok(normalize_summary(materialize_summary(summary, fallback_title)));
      }
...
async fn request_summary(
  ...
  if !status.is_success() {
    return Err(format!("论文摘要接口 HTTP 状态异常：{}，{}", status, response_text));
  }

  let completion = serde_json::from_str::<ChatCompletionResponse>(&response_text)
    .map_err(|error| format!("解析 chat/completions 响应失败：{}；原始响应：{}", error, response_text))?;
...
fn build_summary_messages(document_context: &str) -> serde_json::Value {
  json!([
    {
      "role": "system",
      "content": "You are an expert research reading assistant for a desktop paper reader. Read only the provided paper content and write the result in Simplified Chinese. Be faithful to the source. If a field is not supported by evidence in the text, explicitly say '鏈湪褰撳墠鍐呭涓槑纭鏄?. Output strict JSON only."
    },
    {
      "role": "user",
      "content": format!(
        "请基于下列论文内容生成模块化论文总览，适合在桌面阅读器中展示。\n要求：\n1. 所有字段使用简体中文。\n2. overview 閻?2-4 句概括整篇文章。\n3. abstract 给出忠实压缩版摘要；如果没有识别到摘要段落，也要给出基于全文的摘要说明。\n4. background / researchProblem / approach / experimentSetup / conclusions / limitations 各用 1-3 句。\n5. keyFindings 输出 3-6 条要点。\n6. takeaways 输出 2-4 条给读者的结论性提示。\n7. keywords 输出 4-8 个关键词。\n\n论文内容：\n{}",
        document_context
      )
    }
  ])
...
pub async fn summarize_document_openai_compatible(
  ...
  if blocks.is_empty() && document_text.trim().is_empty() {
    return Err("瑜版挸澧犲▽鈩冩箒閸欘垳鏁ゆ禍搴ｆ晸閹存劖鎲崇憰浣烘畱缂佹挻鐎崠鏍ㄦ瀮閺?.to_string());
  }
  ...
  let document_context = if document_text.trim().is_empty() {
    build_summary_context_from_blocks(
      title,
      authors.as_deref(),
      year.as_deref(),
      &blocks,
    ...
    build_summary_context_from_text(
      title,
      authors.as_deref(),
      year.as_deref(),
      &document_text,
