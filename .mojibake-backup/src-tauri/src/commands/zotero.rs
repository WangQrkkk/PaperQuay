...
fn zotero_headers(api_key: &str) -> Result<HeaderMap, String> {
  ...
  headers.insert(
    ...
    HeaderValue::from_str(api_key.trim())
      .map_err(|error| format!("Zotero API Key 非法：{}", error))?,
  );
...
async fn load_pdf_child(
  ...
  let children = parse_json_response::<Vec<ZoteroApiItem>>(response, "Zotero 瀛愰檮浠?).await?;
...
fn path_to_string(path: PathBuf) -> Result<String, String> {
  path
    .into_os_string()
    .into_string()
    .map_err(|_| "璺緞鍖呭惈鏃犳硶澶勭悊鐨勫瓧绗?.to_string())
}
...
fn resolve_local_zotero_data_dir(input: Option<String>) -> Result<PathBuf, String> {
  if let Some(path) = input {
    ...
    return Err(format!("鐩綍涓病鏈夋壘鍒?zotero.sqlite：{}", data_dir.display()));
  }

  candidate_local_zotero_dirs()
    .into_iter()
    .find(|path| path.join("zotero.sqlite").is_file())
    .ok_or_else(|| "没有找到本地 Zotero 数据目录，请手动选择包含 zotero.sqlite 鐨勭洰褰?.to_string())
}
...
fn copy_sqlite_for_read(data_dir: &Path) -> Result<PathBuf, String> {
  ...
  fs::copy(&source, &target)
    .map_err(|error| format!("复制 Zotero 数据库失败：{}，请确认 Zotero 娌℃湁閿佸畾鏁版嵁搴?, error))?;
...
fn build_local_library_item(
  ...
  let title = local_field_value(connection, metadata_item_id, "title")?
    .or_else(|| local_field_value(connection, attachment_item_id, "title").ok().flatten())
    .unwrap_or_else(|| "鏈懡鍚?PDF".to_string());
...
fn build_note_title(prefix: &str, content: &str) -> String {
  ...
  if snippet.chars().count() > 28 {
    format!("{} 璺?{}閳?, prefix, title)
  } else {
    format!("{} 璺?{}", prefix, title)
  }
}
...
fn build_file_note_title(file_path: &str) -> String {
  PathBuf::from(file_path)
    ...
    .unwrap_or_else(|| "关联笔记".to_string())
}

fn read_text_file_lossy(path: &Path) -> Result<String, String> {
  let bytes = fs::read(path)
    .map_err(|error| format!("读取 Zotero 关联笔记文件失败：{} ({})", error, path.display()))?;
...
fn load_related_file_notes(
  ...
  for row in rows {
    ...
    let Some(file_path) = resolve_local_attachment_path(data_dir, &attachment_key, &raw_path) else {
      continue;
    };
...
pub fn zotero_select_local_data_dir() -> Result<Option<String>, String> {
  FileDialog::new()
    .set_title("选择 Zotero 鏁版嵁鐩綍锛堝寘鍚?zotero.sqlite閿?)
    .pick_folder()
    .map(path_to_string)
    .transpose()
}
...
pub fn zotero_list_local_library_items(
  ...
  let mut statement = connection
    ...
    .map_err(|error| format!("准备 Zotero 本地文献查询失败：{}", error))?;
  let rows = statement
...
pub async fn zotero_lookup_key(api_key: String) -> Result<ZoteroKeyInfo, String> {
  ...
  let user_id = key_info
    .user_id
    .ok_or_else(|| "Zotero API Key 响应缺少 userID".to_string())?
    .to_string();
...
pub async fn zotero_list_library_items(
  ...
  for item in items {
    let item_type = item.data.item_type.clone().unwrap_or_else(|| "unknown".to_string());
    let title = item
      .data
      .title
      .clone()
      .filter(|value| !value.trim().is_empty())
      .unwrap_or_else(|| "閺堫亜鎳￠崥宥嗘瀮閻?.to_string());
    ...
    let attachment_filename = attachment.take().and_then(|attachment| attachment.data.filename);
...
pub async fn zotero_download_attachment_pdf(
  ...
  let client = zotero_client()?;
  let endpoint = format!("{}/users/{}/items/{}/file", ZOTERO_API_BASE, user_id, attachment_key);
  let response = client
    .get(endpoint)
    .headers(zotero_headers(api_key)?)
    .send()
  ...
  if !status.is_success() {
    ...
    return Err(format!("下载 Zotero PDF 附件 HTTP 状态异常：{}，{}", status, text));
  }
  ...
  fs::write(&path, bytes)
    .map_err(|error| format!("写入 Zotero PDF 临时文件失败：{}", error))?;
