...
function isInlineFormulaBoundary(value: string | undefined) {
  return (
    value == null ||
    value === '' ||
    /[\s\u4e00-\u9fff锛屻€傦紱锛氾紒锛熴€侊紙锛夈€愩€戙€娿€嬧€溾€濃€樷€?"`[\](){}<>]/.test(value)
  );
}
...
function canStartInlineFormulaCandidate(value: string, index: number) {
  ...
  return /^(?:[A-Za-z0-9])(?:(?:\s*[_^])|(?:\s*,\s*[A-Za-z0-9])*\s*(?:=|<|>|鈭坾鈭墊鈯倈鈯億鈮鈮鈮坾鈮?|\s*\\(?:in|notin|subset|supset|forall|exists|times|cdot|cup|cap|to|rightarrow)\b|[=<>∈∉⊂⊃≤≥≈≠×·÷])/.test(
    tail,
  );
}
...
function looksLikeInlineFormulaSegment(value: string) {
  ...
  const hasMathOperator =
    /[=<>∈∉⊂⊃≤≥≈≠×·÷]/.test(trimmed) ||
    /\\(?:in|notin|subset|supset|forall|exists|times|cdot|sum|min|max|leq|geq|neq|approx|tag|cup|cap|to|rightarrow)\b/.test(
      trimmed,
    );
  const hasMathSpacing = /~/.test(trimmed);
  const hasSymbolicVariable =
    /(?:^|[\s,(])(?:[A-Za-z\u0370-\u03FF\u1F00-\u1FFF](?:\s*[_^]\s*(?:\{[^{}]+\}|[A-Za-z0-9\u0370-\u03FF\u1F00-\u1FFF]))?)(?:[\s,),]|$)/.test(
      trimmed,
    );
  const hasVariableRelation =
    /(?:^|[\s,(])(?:[A-Za-z\u0370-\u03FF\u1F00-\u1FFF](?:\s*[_^]\s*(?:\{[^{}]+\}|[A-Za-z0-9\u0370-\u03FF\u1F00-\u1FFF]))?)(?:\s*,\s*[A-Za-z\u0370-\u03FF\u1F00-\u1FFF](?:\s*[_^]\s*(?:\{[^{}]+\}|[A-Za-z0-9\u0370-\u03FF\u1F00-\u1FFF]))?)*\s*(?:=|<|>|鈭坾鈭墊鈯倈鈯億鈮鈮鈮坾鈮?/.test(
      trimmed,
    );
  ...
  while (index < protectedLine.length) {
    ...
    while (end < protectedLine.length && INLINE_FORMULA_CHAR_PATTERN.test(protectedLine[end])) {
      if (
        ...
        /^\s+[A-Za-z]{2,}\b/.test(protectedLine.slice(end))
      ) {
        break;
      }
