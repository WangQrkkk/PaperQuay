  ...
  return (
    ...
    >
      <button
        ...
        aria-label={expanded ? `折叠 ${collection.name}` : `展开 ${collection.name}`}
      >
        <ChevronRight
...
function LibraryWorkspace({
  ...
  return (
    <div ref={containerRef} className="flex h-full min-h-0">
          ...
          <>
            ...
            <div className="min-h-0 flex-1 overflow-y-auto px-3 py-4">
              <div className="space-y-2">
                {[
                  ['recent', Clock3, '鏈€杩戞坊鍔?],
                  ['all', Files, '全部 PDF'],
                  ['standalone', BookOpenText, '独立 PDF'],
                ].map(([key, Icon, label]) => (
                  <button
                    key={key as string}
                    type="button"
          ...
          <>
                ...
                <div className="space-y-1">
                  {flattenedCollections.length === 0 ? (
                    <div className="rounded-2xl border border-dashed border-slate-200 bg-slate-50/80 px-4 py-4 text-sm leading-6 text-slate-500">
                      {libraryLoading ? '正在读取本地 Zotero 鍒嗙被鈥? : '当前没有可显示的 Zotero 鍒嗙被銆?}
                    </div>
                  ) : (
            ...
            <div className="border-t border-slate-200/80 px-4 py-3">
              <div className="rounded-2xl border border-slate-200 bg-white px-3 py-2 text-xs leading-5 text-slate-500">
                {libraryLoading ? '濮濓絽婀崥灞绢劄閺堫剙婀撮弬鍥х氨閳? : statusMessage}
              </div>
            </div>
          </>
        )}
      ...
      >
        <div className="sticky top-0 z-10 border-b border-slate-200/80 bg-white/72 px-5 py-4 backdrop-blur-xl">
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div className="min-w-0">
              <div className="text-xs font-semibold uppercase tracking-[0.18em] text-slate-400">
                {selectedSectionKey.startsWith('collection:') ? 'Collection' : 'Library'}
              </div>
              <div className="mt-1 text-xl font-semibold tracking-tight text-slate-950">
                {selectedSectionTitle}
              </div>
              <div className="mt-2 text-sm text-slate-500">閸欏苯鍤拋鐑樻瀮閸楀啿褰查崷銊︽煀閺嶅洨顒锋い鍏歌厬閹垫挸绱戦梼鍛邦嚢閸ｃ劊鈧?/div>
            </div>
                      ...
                      >
                        {hasChildren ? (
                          <button
                            ...
                            aria-label={expanded ? '折叠论文附件' : '展开论文附件'}
                          >
                            <ChevronRight
                              className={clsx('h-4 w-4 transition-transform duration-200', expanded && 'rotate-90')}
                              strokeWidth={1.9}
                          ...
                          return (
                            ...
                            >
                              ...
                              <div className="ml-3 flex shrink-0 flex-col items-end gap-1">
                                ...
                                >
                                  {itemParseStatusMap[item.workspaceId] ? '瀹歌尪袙閺? : '閺堫亣袙閺?}
                                </span>
                              </div>
                            </button>
                          );
      ...
      <section className="min-h-0 min-w-0 flex-1 border-l border-slate-200/80">
        {previewPane}
      </section>
    </div>
  );
