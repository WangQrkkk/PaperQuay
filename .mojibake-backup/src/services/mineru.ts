...
import { normalizeLatexExpression } from '../utils/markdown';
...
function extractMathText(input: unknown): string {
  ...
  for (const candidate of candidates) {
    if (typeof candidate === 'string' && candidate.trim()) {
      return normalizeLatexExpression(candidate);
    }
  }

  return normalizeLatexExpression(joinReadableText(collectTextParts(input)));
}
  ...
  switch (block.type) {
    ...
    case 'list':
      return (structuredMarkdown || safeText)
        ...
        .join('\n');
    case 'equation':
      return structuredMarkdown || `$$\n${extractMathText(block.content) || safeText}\n$$`;
    case 'image':
      return `**閸ュ墽澧栫拠瀛樻閿?* ${structuredMarkdown || safeText}`;
    case 'table':
      return `**鐞涖劍鐗哥拠瀛樻閿?* ${structuredMarkdown || safeText}`;
    case 'caption':
      return `> ${structuredMarkdown || safeText}`;
    default:
      return structuredMarkdown || safeText;
...
function normalizeMineruBlock(
  ...
  if (!input || typeof input !== 'object') {
    throw new Error(`缁?${pageIndex + 1} 页第 ${blockIndex + 1} 个块不是对象。`);
  }
...
function parseMiddleJson(raw: Record<string, unknown>): MineruPage[] {
  if (!Array.isArray(raw.pdf_info)) {
    throw new Error('MinerU middle JSON 缺少 pdf_info 鏁扮粍銆?);
  }
...
export function parseMineruPages(payload: string | unknown): MineruPage[] {
    ...
    if (parsed.every(Array.isArray)) {
      return parsed.map((page, pageIndex) => {
        if (!Array.isArray(page)) {
          throw new Error(`缁?${pageIndex + 1} 页不是块数组。`);
        }
  ...
  throw new Error('MinerU JSON 蹇呴』鏄?content_list_v2、content_list 閹?middle JSON閵?);
}
