...
借助 MinerU，PaperQuay 可以将 PDF 解析为段落、标题、列表、公式、表格和页面区域等结构化内容。软件通过 `blockId + pageIndex + bbox` 建立 PDF 区域和结构化文本之间的关联，从而支持原文、结构块、翻译和概览之间的跳转。
...
| 论文概览     | 支持背景、研究问题、方法、实验设置、发现、结论和局限等概览字段                    |
