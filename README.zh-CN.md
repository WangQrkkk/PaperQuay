...
PaperQuay 内置独立的 Notes 工作区，编辑器基于 Tiptap。笔记会在本地保存 Tiptap JSON、渲染 HTML 和用于搜索的纯文本，同时支持 `[[笔记]]` 双向链接、`#标签`、`@paper` 文献引用、反向链接和本地自动保存。它的目标不是做一个孤立的笔记软件，而是让研究笔记始终和论文、PDF 阅读与文献库保持连接。
...
| 3. 整理文献库 | 创建分类，将论文拖入 collection，添加标签并标记收藏。 |
...
| 9. 写笔记 | 创建 Tiptap 富文本笔记，用 `[[标题]]` 连接笔记，用 `#标签` 组织主题，并引用文献。 |
...
| 模块 | 已支持能力 |
| ---- | ---------- |
| 本地文献库 | 使用本地 SQLite 保存论文、作者、分类、标签、附件、笔记、批注、导入记录和 RAG 索引 |
...
| 笔记工作区 | 支持 Tiptap 富文本编辑、本地自动保存、全文搜索、标签、双向链接、文献引用、大纲和反向链接 |
| 阅读器 | 支持 PDF 阅读、MinerU 结构块视图、PDF 区域联动、阅读热力进度和阅读时间记录 |
